...
export default async function handler(req, res){
  ...
  const update = {};
  if (typeof display_name !== 'undefined') update.display_name = display_name || null;
  if (typeof email !== 'undefined') update.email = email || null;

  try {
    const { error } = await supabase.from('users').update(update).eq('id', payload.user_id);
